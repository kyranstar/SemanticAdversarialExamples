...
class MnistWganInv(object):
    def __init__(self, x_dim=784, z_dim=64, latent_dim=64, batch_size=80,
        ...
        self.z = tf.compat.v1.placeholder(tf.float32, shape=[None, self.z_dim])
        self.x_p = self.generate(self.z)

        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, self.x_dim])
        ...
        self.gen_cost = -tf.reduce_mean(input_tensor=self.dis_x_p)
        tf.compat.v1.summary.scalar('gen_cost', self.gen_cost)

        self.inv_cost = tf.reduce_mean(input_tensor=tf.square(self.x - self.rec_x))
        self.inv_cost += self.lamda * tf.reduce_mean(input_tensor=tf.square(self.z - self.rec_z))
        tf.compat.v1.summary.scalar('inv_cost', self.inv_cost)

        self.dis_cost = tf.reduce_mean(input_tensor=self.dis_x_p) - tf.reduce_mean(input_tensor=self.dis_x)
        tf.compat.v1.summary.scalar('dis_cost', self.dis_cost)
        ...
        gradient = tf.gradients(ys=self.discriminate(interpolate), xs=[interpolate])[0]
        slope = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(gradient), axis=1))
        gradient_penalty = tf.reduce_mean(input_tensor=(slope - 1.) ** 2)
        ...
        self.inv_train_op = tf.compat.v1.train.AdamOptimizer(
            learning_rate=1e-4, beta1=0.9, beta2=0.999).minimize(
            self.inv_cost, var_list=self.inv_params)
        self.dis_train_op = tf.compat.v1.train.AdamOptimizer(
            learning_rate=1e-4, beta1=0.9, beta2=0.999).minimize(
...
if __name__ == '__main__':
    ...
    tf.compat.v1.set_random_seed(326)
    np.random.seed(326)
    ...
    with tf.compat.v1.Session() as session:
        ...
        train_writer = tf.compat.v1.summary.FileWriter('summaries/train_mnist', session.graph)
