...
class MnistWganInv(object):
    ...
    def generate(self, z):
        ...
        output = tf.keras.layers.Conv3DTranspose(filters=4,
                                        kernel_size=8,
                                        strides=(4,4,4),
                                        padding='SAME',
    ...
    def discriminate(self, x):
        ...
        output = tflib.ops.linear.Linear(
            'Discriminator.Output', self.latent_dim * 36, 1, output)
    ...
    def invert(self, x):
        ...
        output = tflib.ops.linear.Linear(
            'Inverter.4', self.latent_dim * 36, self.latent_dim * 4, output)
        output = tf.nn.leaky_relu(output)

        output = tflib.ops.linear.Linear(
            'Inverter.Output', self.latent_dim * 4, self.z_dim, output)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--z_dim', type=int, default=48, help='dimension of z')
